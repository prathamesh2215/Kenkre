...
if((isset($obj->load_area)) == "1" && isset($obj->load_area))
{
	...
	if($page != "" && $per_page != "")	
	{
		...
		$sql_load_data  = " SELECT `id`, `area_name`, `area_pincode`, `created_date`, `created_by`, `modified_date`, `modified_by`, `status` ";
		$sql_load_data  .= " FROM `tbl_area` AS ta WHERE 1=1 ";
		if(strcmp($utype,'1') !== 0)
		{
		...
		if($search_text != "")
		{
			$sql_load_data .= " and (area_name like '%".$search_text."%' or area_pincode = '".$search_text."' or id = '".$search_text."') ";
		}
