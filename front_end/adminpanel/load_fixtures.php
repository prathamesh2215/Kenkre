	...
	if($page != "" && $per_page != "")	
	{
		...
		{		
			...
			$area_data .= '<table id="tbl_user" class="table table-bordered dataTable" style="width:100%;text-align:center">';
         	...
         	$area_data .= '<th style="text-align:center">Fixtures ID</th>';
			...
			while($row_load_data = mysqli_fetch_array($result_load_data))
			{
			   ...
			   while($row = mysqli_fetch_array($res_get_fixture))
			   {
			   		if($i!=0)
			   		{
			   			...
			   			$area_data .= '<td>'.$row['fixture_id'].'</td>';
					    ...
					    $area_data .= '<td  style="text-align:center">'.ucwords($row['fixture_time_start']).' to '.ucwords($row['fixture_time_end']).'</td>';
					    		...
					    		// query for getting the team B name 
					    		$area_data .= '<td style="text-align:center">'.strtoupper($team_a_name).'</td>';		
								$area_data .= '<td style="text-align:center">'.strtoupper($team_b_name).'</td>';		
...
if((isset($obj->load_fixture_parts)) == "1" && isset($obj->load_fixture_parts))
{
	...
	if($req_type != "")
	{
		...
		if($req_type !="view")
		{
			...
			// ============================================================================================================
			// START : Team A and Team B DDL 
			// ============================================================================================================

			$data .= '<div class="control-group">';
				$data .= '<label for="tasktitel" class="control-label">Teams<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
				$data .= '<div class="controls">';
					$data .= '<select id="team_a" name="team_a" class="select2-me input-large">';
						$data .= '<option value="">Select Team A</option>';
						// Query for getting the list of Team A
						$sql_get_list_team_a = " SELECT `team_id`,`team_name`,`pool` FROM `tbl_team` WHERE 1  ";
						$res_get_list_team_a = mysqli_query($db_con, $sql_get_list_team_a) or die(mysqli_error($db_con));
						while($row_get_list_team_a = mysqli_fetch_array($res_get_list_team_a))
						{
							$data .= '<option value="'.$row_get_list_team_a['team_id'].'" ';

							if($fixture_id != "" && $req_type == "edit")
							{
								if($row_get_list_team_a['team_id'] == $row_fixture_data['team_a'])
								{
									$data .= 'selected';
								}
							}
							...
							$data .= ' >'.strtoupper($row_get_list_team_a['team_name']).' ('.strtoupper($row_get_list_team_a['pool']).') </option>';
						}
					$data .= '</select>';
					$data .= '&nbsp;&nbsp;to&nbsp;&nbsp;';
					$data .= '<select id="team_b" name="team_b" class="select2-me input-large">';
						$data .= '<option value="">Select Team B</option>';
						// Query for getting the list of Team A
						$sql_get_list_team_b = " SELECT `team_id`,`team_name`,`pool` FROM `tbl_team` WHERE 1  ";
						$res_get_list_team_b = mysqli_query($db_con, $sql_get_list_team_b) or die(mysqli_error($db_con));
						while($row_get_list_team_b = mysqli_fetch_array($res_get_list_team_b))
						{
							$data .= '<option value="'.$row_get_list_team_b['team_id'].'" ';
							if($fixture_id != "" && $req_type == "edit")
							{
								if($row_get_list_team_b['team_id'] == $row_fixture_data['team_b'])
								{
									$data .= 'selected';
								}
							}

							$data .= '>'.strtoupper($row_get_list_team_b['team_name']).' ('.strtoupper($row_get_list_team_b['pool']).') </option>';
						}
					$data .= '</select>';
				$data .= '</div>';
			$data .= '</div>';
			...
			$data .= '<div class="control-group">';
				$data .= '<label for="tasktitel" class="control-label">Teams<sup class="validfield"><span style="color:#F00;font-size:20px;">*</span></sup></label>';
				$data .= '<div class="controls">';
					$data .= '<select id="pool" name="pool" class="select2-me input-large">';
						$data .= '<option value="">Select POOL</option>';
						// Query For getting all the POOLs
						$sql_get_list_of_pools = " SELECT DISTINCT(`pool`) FROM `tbl_fixtures` WHERE 1 ORDER BY `tbl_fixtures`.`pool` ASC ";
						$res_get_list_of_pools	= mysqli_query($db_con, $sql_get_list_of_pools) or die(mysqli_error($db_con));
						while($row_get_list_of_pools = mysqli_fetch_array($res_get_list_of_pools))
						{
							$data .= '<option value="'.$row_get_list_of_pools['pool'].'" ';
							if($fixture_id != "" && $req_type == "edit")
							{
								if($row_get_list_of_pools['pool'] == $row_fixture_data['pool'])
								{
									$data .= ' selected ';
								}
							}
							$data .= '>'.strtoupper($row_get_list_of_pools['pool']).'</option>';
						}
					$data .= '</select>';
				$data .= '</div>';
			$data .= '</div>';
